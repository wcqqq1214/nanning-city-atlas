...
| 菜单 | 行为 |
| --- | --- |
| 探索 | 全景鸟瞰；访问 21 个探索点，覆盖石埠、相思湖、老城、校园、文化场馆、天际线、山林、桥梁与交通枢纽 |
...
自动模式会为手机与节省流量偏好选择轻量模型；也可在“环境 → 画面偏好”手动切换。轻量版约 5.79 MB，相对精细版文件减少 30.3%，三角形减少 27.8%，保留全部建筑、道路、水面和地标。场景按空间分组，近景时剔除视野外的组。
...
城区及五象新区：三街两巷 · 畅游阁、人民公园 · 镇宁炮台、广西大学 · 汇学堂、广西博物馆、南湖公园、地王大厦、国际会展中心、华润大厦、南宁东站、广西民族博物馆、南宁孔庙、青秀山 · 龙象塔、南宁大桥、广西文化艺术中心、广西体育中心、亭子码头。

其中 16 个点有独立地标模型，其余 5 个以公开地图中的校园、湖泊、绿地和周边街区为主体。每个点配置独立的镜头距离；探索菜单、地图标记和漫游使用同一份 [地标目录](data/landmarks.json)。目录中的坐标来源及近似方式见 [数据说明](docs/DATA_SOURCES.md)。
...
- 10,398 个道路片段；精细版保留 3,361 棵常规单树，流畅版保留 865 棵。另以约 213.88 km² 连片树冠代替 7,600 棵林内示意树，两档均保留南湖的 83 棵阔叶树和 36 棵棕榈。
...
- `public/models/nanning-city.glb`：8.32 MB 的 Draco 精细模型。
- `public/models/nanning-city-mobile.glb`：5.79 MB 的 Draco 轻量模型，保留完整建筑、道路、水系及地标。
