...
自动模式会为手机与节省流量偏好选择轻量模型；也可在“环境 → 画面偏好”手动切换。轻量版文件减少 55.0%，三角形减少 58.7%，保留全部建筑、道路、水面和地标。场景按空间分组，近景时剔除视野外的组。
...
- `public/models/nanning-city.glb`：7.66 MB 的 Draco 精细模型。
- `public/models/nanning-city-mobile.glb`：3.45 MB 的 Draco 轻量模型，保留完整建筑、道路、水系及地标。
