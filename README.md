...
其中 15 个点有独立的简化地标模型，其余 6 个以公开地图中的校园、湖泊、绿地和周边街区为主体。每个点配置独立的镜头距离；探索菜单、地图标记和漫游使用同一份 [地标目录](data/landmarks.json)。目录中的坐标来源及近似方式见 [数据说明](docs/DATA_SOURCES.md)。
...
- 10,398 个道路片段；精细版显示 10,983 棵树，流畅版显示 2,746 棵树。
...
- `public/models/nanning-city.glb`：9.05 MB 的 Draco 精细模型。
- `public/models/nanning-city-mobile.glb`：4.83 MB 的 Draco 轻量模型，保留完整建筑、道路、水系及地标。
