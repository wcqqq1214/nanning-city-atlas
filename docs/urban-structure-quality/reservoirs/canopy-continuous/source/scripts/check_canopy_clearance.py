...
def check(canopy, terrain, minimum_clearance_meters=9.5):
    roof = TerrainSurface(canopy)
    source_ids = np.flatnonzero(np.abs(np.cross(canopy[:, 1]-canopy[:, 0], canopy[:, 2]-canopy[:, 0])[:, 2]) > 1e-12)
    ...
    passed = (failed == 0 and total_missing <= .01 and roof.ignoredVerticalOrDegenerateFaces == 0)
    return {'passed': passed, 'scope': 'continuous native mesh overlap; compressed export and visual QA are separate',
            ...
            'missingCoverageSquareMeters': total_missing, 'maximumFaceMissingCoverageSquareMeters': maximum_missing,
            'facesMissingMoreThanOneSquareCentimeter': missing_faces,
...
def main():
    ...
    result = check(np.load(args.canopy)['triangles'], np.load(args.terrain)['triangles'], args.minimum_clearance_meters)
