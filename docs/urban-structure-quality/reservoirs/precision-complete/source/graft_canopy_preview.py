...
b=root/'work/urban-structure/p5/full-city-review';stage=root/'work/urban-structure/p5/reservoirs/integration/budget/canopy-conforming-staging';out=b/'global-conforming-preview';out.mkdir(exist_ok=False)
...
for profile,filename in [('detail','nanning-city.glb'),('smooth','nanning-city-mobile.glb')]:
 ...
 for region in ['nearby','all']:
  p=b/'global-conforming-native'/(region+'-'+profile+'.glb');d,blobs=read(p);inputs[str(p)]=digest(p)
