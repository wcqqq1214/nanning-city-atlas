...
class Tests(unittest.TestCase):
    def test_dam_perimeter_excludes_internal_crest_seams(self):
        from prepare_reservoir_terrain import dam_boundary_vertices
        # Four boundary stations surround an interior crest station. Material
        # changes within the dam must not turn that station into a fixed toe.
        faces=[[0,1,4],[1,2,4],[2,3,4],[3,0,4],[0,5,1]]
        keys=['dam_slope','dam_crest','dam_crest','dam_slope','reservoir_ground']
        self.assertEqual(dam_boundary_vertices(faces,keys),{0,1,2,3})
        self.assertEqual(dam_boundary_vertices(faces,['reservoir_ground']*5),set())
        with self.assertRaises(ValueError):
            dam_boundary_vertices([[0,1,2],[1,0,3],[0,1,4]],['dam_slope']*3)
