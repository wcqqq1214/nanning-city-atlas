...
p=b/'canopy-conforming-native/nearby-smooth.npz'
...
records.sort(key=lambda r:-abs(r['heightGapMeters']));(b/'nanhu-slit-conforming-edges.json').write_text(json.dumps({'source':str(p),'triangles':len(f),'mismatches':records},indent=2)+'\n');print(len(f),len(records));print(records[:5])
