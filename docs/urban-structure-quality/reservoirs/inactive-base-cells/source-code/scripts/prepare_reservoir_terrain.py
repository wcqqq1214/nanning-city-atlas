...
def crest_polygon(footprint, width_meters, inset_meters):
    ...
    axis = LineString([center-edge, center+edge]).intersection(footprint.buffer(-inset_meters/100))
    ...
    crest = axis.buffer(width_meters/200, cap_style=2, join_style=2).intersection(footprint)
...
def prepare(geo, dem, source, raster_path):
    ...
    for point,z,protected_weight in zip(points,native_z,protection):
