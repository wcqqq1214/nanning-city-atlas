...
def recut(mesh, terrain, domain, partition_tolerance_meters=0):
    ...
    ground_shapes = shapely.polygons(partition[:, :, :2])
    ground_tree = STRtree(ground_shapes)
    ...
    for i in ground_tree.query(allowed, predicate='intersects'):
        part = ground_shapes[int(i)].intersection(allowed)
        if part.area <= 1e-12:
            continue
    ...
    after_shapes = shapely.polygons(np.asarray(points)[triangles][:, :, :2])
    ...
    return result, {'changed': True, 'originalTriangles': len(original), 'candidateTriangles': len(triangles),
...
def main():
    ...
    sources = [args.forest, args.registry, Path(__file__).resolve()]
    domains = []
    ...
    for profile in ['detail', 'smooth']:
        ...
        for region in plan['regions']:
            try:
                region[profile], record = recut(region[profile], terrain, domain, args.partition_tolerance_meters)
