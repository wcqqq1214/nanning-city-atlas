...
下列单树数量、体积和对比记录对应林地扩展完成时。后续两座车站替换站场内的普通树木，当前数量与总模型预算见 [MOBILE.md](MOBILE.md)，林冠覆盖与冠簇保持不变。

## 相对青秀山样例版的阶段变化

| 全场景 | 青秀山样例版 | 林地扩展完成时 | 变化 |
| --- | ---: | ---: | ---: |
...
此阶段两档在原有 10 MB / 5.8 MB 下载预算内。流畅版空间用于扩大连续森林覆盖，不能把精细版的降幅套用于手机性能。类型检查、lint、5 项点触测试、地理与模型资产验证、静态构建均通过；未进行手机真机帧率或发热测试。
