...
4. 普通道路按类型设置示意宽度，并按显示地形采样高度。普通桥梁抬升，隧道不在地表绘制。清厢快速路城区高架使用保留节点连接的双向主线与八条匝道，生成约 6.66 km 连续箱梁、桥墩和地面衔接。匝道落地点与周围引道共用高度缓冲，避开两档显示地形；宽度、墩距与高程为展示近似，详见 [高架建模](VIADUCT.md)。
