...
def validate_zhuxi():
    ...
    for profile,filename in [('detail','nanning-city.glb'),('smooth','nanning-city-mobile.glb')]:
        ...
        model=json.loads(raw[20:20+size]);nodes=model['nodes'];groups={'lamps':[],'trees':[],'roads':[]}
        ...
        for node in nodes:
            ...
            for p in model['meshes'][node['mesh']]['primitives']:
                ...
                if key=='roads':
                    c=tri.mean(axis=1);tri=tri[(c[:,0]>73)&(c[:,0]<80)&(c[:,1]>-14)&(c[:,1]<-6)]
        ...
        for name in ['lamps','trees']:
            conflicts=intersections(groups[name],groups['roads'],profile+' Zhuxi '+name+'/road')
            assert conflicts['pairs']==0,f'Zhuxi {name} intersects road surface'
