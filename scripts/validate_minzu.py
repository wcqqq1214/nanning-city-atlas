...
def validate_minzu():
    ...
    for route in plan['paths'] + plan['tunnels']:
        e = selected[route['osmId']]
        assert e['tags'] == route['tags']
        retained[e['id']].append(LineString(route['points']))
    ...
    replacements = [r['roadIndex'] for r in plan['paths']]
    ...
    for filename in ['nanning-city.glb','nanning-city-mobile.glb']:
        ...
        counts = {'structure':0,'details':0}; asphalt = []; marking_faces=collapsed=0
        minzu_materials = set()
        ...
        print(f'{filename} Minzu: {counts}; markings collapsed {collapsed}/{marking_faces}; source gap {max_distance*100:.3f} m',flush=True)
    ...
    print('PASS: Minzu original ways, legacy replacement, terrain/grade report, shared materials, road layer and decoded markings.',flush=True)
