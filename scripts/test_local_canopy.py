...
class LocalCanopyTests(unittest.TestCase):
    def test_partial_recut_nodes_the_unreplaced_side_of_its_boundary(self):
        from unittest.mock import patch
        mesh = {'points': [[0, -1, .1], [2, -1, .1], [2, 1, .1], [0, 1, .1]],
                'triangles': [[0, 1, 2], [0, 2, 3]], 'colors': [1, 1]}
        terrain = []
        for west, east in [(0, .5), (.5, 2)]:
            quad = [[west, 0, 0], [east, 0, 0], [east, 1, 0], [west, 1, 0]]
            terrain.extend([[quad[i] for i in face] for face in [[0, 1, 2], [0, 2, 3]]])
        def crosses_boundary_station(candidate):
            points = np.asarray(candidate['points'])
            for face in candidate['triangles']:
                for ai, bi in zip(face, face[1:]+face[:1]):
                    a, b = points[ai], points[bi]
                    if abs(a[1])+abs(b[1]) < 1e-12 and min(a[0], b[0]) < .5 < max(a[0], b[0]):
                        return True
            return False
        with patch('prepare_local_canopy.conform_edges', side_effect=lambda mesh, *a, **k: (mesh, {})):
            old, _ = recut(mesh, np.asarray(terrain), box(0, 0, 2, 1))
        self.assertTrue(crosses_boundary_station(old))
        candidate, record = recut(mesh, np.asarray(terrain), box(0, 0, 2, 1))
        self.assertFalse(crosses_boundary_station(candidate))
        self.assertLess(record['coverageDifferenceSquareMeters'], 1e-8)

    def test_subsquaremillimetre_cut_fragments_obey_total_coverage_gate(self):
        mesh = {'points': [[0, 0, .1], [1, 0, .1], [1, 1, .1], [0, 1, .1]],
                'triangles': [[0, 1, 2], [0, 2, 3]], 'colors': [1, 1]}
        terrain = []
        for west, east in [(0, 5e-11), (5e-11, 1)]:
            quad = [[west, 0, 0], [east, 0, 0], [east, 1, 0], [west, 1, 0]]
            terrain.extend([[quad[i] for i in face] for face in [[0, 1, 2], [0, 2, 3]]])
        candidate, record = recut(mesh, np.asarray(terrain), box(0, 0, 1, 1))
        faces = np.asarray(candidate['points'])[candidate['triangles']]
        areas = np.abs(np.cross(faces[:, 1]-faces[:, 0], faces[:, 2]-faces[:, 0])[:, 2])/2
        self.assertTrue(np.all(areas > 1e-10))
        self.assertGreater(record['coverageDifferenceSquareMeters'], 0)
        self.assertLess(record['coverageDifferenceSquareMeters'], .01)
