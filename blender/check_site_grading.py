...
def main():
    ...
    result['tools']={str(f.relative_to(ROOT)):hashlib.sha256(f.read_bytes()).hexdigest() for f in
                     [Path(__file__).resolve(),ROOT/'blender/site_grading.py',ROOT/'blender/terrain_mesh.py',ROOT/'blender/forest_canopy.py',ROOT/'blender/build_city.py',ROOT/'blender/site_access_plan.py',ROOT/'blender/nanhu_terrain.py',ROOT/'blender/nanhu_landmark.py',ROOT/'blender/reduced_surface.py',ROOT/'blender/scene_ground.py']}
    (args.output/'report.json').write_text(json.dumps(result,indent=2)+'\n')
